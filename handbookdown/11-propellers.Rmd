...
|||
|:-|:----------|
|\(\mathrm{AF}\)|activity factor *=*\* *![][11001]|
...
|\(C_{L_i}\)|integrated design lift coefficient = ![][11002]|
|\(C_P\)|power (absorbed) coefficient|
|\(C_T\)|thrust coefficient|
|\(D\)|propeller diameter (feet)|
|\(f_c\)|ratio of speed of sound at standard day sea level to speed of sound at operating condition \* *![][11003]|
|\(\mathrm{GR}\)|gear ratio, propeller speed/engine speed|
|\(\mathrm{HP}\)|horsepower (1 \(\mathrm{HP}\) = 550 ft-lb/sec)|
...
|\(N\)|propeller speed, revolutions per minute (\(\text{RPM}\))|
|\(n\)|propeller speed, revolutions per second|
|\(N_e\)|engine speed, \(\text{RPM}\)|
...
|\(V_K\)|freestream velocity (knots)|
|\(V_R\)|resultant velocity|
|\(V_T\)|freestream velocity (ft/sec)|
|\(V_{\text{tan}}\)|tangential velocity|
|\(V_{\text{tip}}\)|tip speed|
|\(x\)|fraction of propeller tip radius, *r/R*|
|\(\alpha\)|local angle of attack|
|\(\beta\)|local blade twist angle, measured between chord and plane of rotation, same as *θ* (degrees).|
|\(\eta\)|isolated propeller efficiency.|
|\(\eta_{\text{comp}}\)|composite prop efficiency (includes tip and blockage corrections)|
|\(\theta^{\frac{3}{4}}\)|propeller blade twist angle at \(x\) =3/4 (degrees), same as *β ^3^*^/4^|
|\(\sigma\)|ratio of operating density to sea level standard density = *ρ*~a~/*ρ*~0.~|
|\(\phi\)|propeller disk angle of attack|
|\(\omega\)|propeller rotation speed (radians/second)|
...
thickness ratio (*TR*) blade thickness measured locally or at  0.75\(R\) to represent entire prop.
