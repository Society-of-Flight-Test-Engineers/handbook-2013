...
\[c_p = \left( p_{\text{local}} - p_{\text{ambient}} \right) / \text{dynamic pressure} = \Delta p/q \]
...
\[\delta_{lam} = \frac{5.2x}{\sqrt{\mathrm{Re}_x}} \]

\[\delta_{turb} = \frac{0.37x}{\mathrm{Re}_x^{0.2}} \]
...
\[C_{P_{\text{compressible}}} = \frac{1}{\sqrt{1-M^2}}C_{P_{\text{incompressible}}} \]
...
\(S_{\text{wet}} = \text{wetted area of surface}\)
...
\[C_D = C_{D_{\text{min}}} + \frac{\left(C_L - C_{L_{\text{min}}}\right)^2}{\pi \mathrm{AR} e} +k_2\left(C_L - C_{L_{\text{break}}}\right)\]

* Delete last term if \(C_L < C_{L_{\text{break}}} \)
...
* Friction drag \(= c_f q S_{\text{wet}}\) for each component (\(S_{\text{wet}} = \text{wetted area}\))
* Correct from test day to standard day aircraft drag coefficient by summing differences of each component's drag change

\[\Delta C_D = \frac{\sum\left(c_{f_s} - c_{f_t} \right) S_{\text{wet}}}{S} \]
...
\[C_D = C_{D_{\text{min}}} + \frac{\left(C_L - C_{L_{\text{min}}} \right)^2}{\pi \mathrm{AR} e} \]

* Generally not necessary since most flight occurs above \(C_{L_{\text{min}}}\)
...
\[C_{D_{\text{trim}}} = \frac{W^2}{\pi q^2 Sb^2e} \left[\frac{2}{lW}\left[x_0 - x_1\right] + \frac{1}{l^2} \left[1 + \frac{S}{S_t} \frac{e}{e_t} \left(\frac{b}{b_t}\right)^2\right] \left[x_0^2 - x_1^2\right] \right]\]

Trim drag change relative to total induced drag:

\[\frac{\Delta C_{D_{\text{trim}}}}{\Delta C_{D_i}} = \frac{x}{l} \left[\frac{x}{l} \left(\frac{b}{b_t}\right)^2 \frac{e}{e_t} - 2 \right] \]
...
* Minimum total drag occurs when \(D_{\text{induced}} = D_{\text{parasitic}}\)
...
Minimum total \(P_{\text{req'd}}\) occurs when \(P_{\text{induced}} = P_{\text{parasitic}}\)

*  same as speed where \(C_{D_i} = 3C_{D_0} \)
*  occurs at max \(C_L^{3/2}/C_D\) ratio

Minimum power/velocity occurs at min slope of \(P_{\text{req'd}}\) vs \(V\) curve
