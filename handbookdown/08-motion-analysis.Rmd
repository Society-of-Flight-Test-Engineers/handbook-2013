...
when *t* = 0.6931τ: *x = e*^− 0.6931^ = 0.5 (time to half amplitude)
...
• *x*~(t2)~ = 2*x*~(0)~ where *x*~(0)~ = *c\mathrm{e}^m^*^0^
...
### Determining *τ* from Step Input Time History
...
2\. Draw tangent at *t_1\).

3\. Note *t_2\) where tangent intercepts x~ss~.
...
Divide out *\mathrm{e}^st^*, since it never equals zero, the characteristic equation remains:
