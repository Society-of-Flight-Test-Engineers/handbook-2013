...
\[ x = c_1 e^{s_1 t} + c_2 e^{s_2 t} \text{ where } s_1,s_2 = -\frac{D}{2M} \pm j \sqrt{\frac{K}{M} - \left( \frac{D}{2M} \right)^2} \]

Apply Euler's identity for complex conjugate roots

\[ x = A e^{-\frac{D}{2M}} \sin \left( \sqrt{\frac{K}{M} - \left( \frac{D}{2M} \right)^2} t + \phi \right) \]
...
* If damping is reduced to \(D = 0\) then only \(\sqrt{\frac{K}{M}}\) remains. This is the undamped or **"natural" frequency** ( \(\omega_n\) ).
...
\[ D_{\mathrm{crit}} = \sqrt{\frac{K}{M}} 2 M = 2 \sqrt{KM} \]

* For oscillatory motion, actual system damping is typically expressed as a fraction of critical damping. Define **damping ratio** as
...
\begin{align}
x &= A e^{-\frac{D}{2M}} \sin \left( \sqrt{\frac{K}{M} - \left( \frac{D}{2M} \right)^2} t + \phi \right) \\
&= A e^{-\zeta \omega_n t} \sin \left( \omega_d + \phi \right) \\
&= A e^{-\zeta \omega_n t} \sin \left( \omega_n \sqrt{1 - \zeta^2} + \phi \right) \\
...
Settling Time, *T_s\): time to reach some defined % of final value
...
•  Form the time ratios *t_2~/t_1\), *t_3~/t_1\), and \[*t_3~-t_2\)\]/\[*t_2~-t_1\)\]
