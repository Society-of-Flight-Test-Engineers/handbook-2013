...
• [In all cases, test day \(\mathrm{RPM}\) must equal standard day \(\mathrm{RPM}\)]{.underline}. There are no corrections if this requirement is violated.
...
**Partial Throttle Standardization.** If the test engine is set at some partial throttle setting to achieve a specific manifold pressure, then the same \(\mathrm{MP}\) should be achievable on standard day with a slightly different throttle setting. Calculate standard day power (*BHP~s~*) as
