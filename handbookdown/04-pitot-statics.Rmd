...
\[V_{c} = \left\lbrack \frac{2\gamma}{\gamma - 1}\frac{P_{o}}{\rho_{o}}\left( \left\lbrack \frac{P_{T} - P_{a}}{P_{o}} + 1 \right\rbrack^{\frac{\gamma - 1}{\gamma}} - 1 \right) \right\rbrack^{\frac{1}{2}}\]


\[\sqrt{\mathrm{7}\frac{\mathrm{P}_{\mathrm{o}}}{\mathrm{\rho}_{\mathrm{o}}}\left( \left\lbrack \frac{\mathrm{q}_{\mathrm{c}}}{\mathrm{P}_{\mathrm{o}}}\mathrm{+ 1} \right\rbrack^{\mathrm{0.2857}}\mathrm{- 1} \right)}\]
...
Equivalent Airspeed (valid if \(q_c/P_a > 0.892929158 \))
...
During position error flight testing, measure \(T_i\).  From \(V_c\) and \(H_{pc}\) determine M

\[ T_i + \Delta T_{ic} = T_T = T_a + T_aKM^2 \]
...
\(P_a = P_0 \left(1-6.87535 \times 10^{-6} H\right)^{5.256}\) below 36,089 ft

\(P_a = P_0 \left(0.22335 \right) \textrm{e}^{-0.00004806[H-36089]}\) above 36,089 ft
...
| \(\Delta H_{ic}\)           | instrument error correction                  | <!--- already defined at the beginning of the section --->
| \(H_{ic}\)                  | altimeter corrected of instrument error      |
| \(\Delta H_{pc}\)           | position error correction                    | <!--- already defined at the beginning of the section --->
...
\[\textrm{Actual } H_{c} = H_{c_{tower}} + \left( D\tan\theta \cdot \frac{T_{s}}{T_{t}} \right) \]

\[ \Delta H_{pc} = \textrm{Actual } H_c - \left( H_i + \Delta H_{ic} \right) \]

<!--- Changing capital subscript 's' to lower-case to make more consistent with rest of the handbook --->
\[ \Delta P_s = \rho g \Delta H_{pc} \]

\[ \Delta P_{S} = q_{c} - q_{cic} \]

\[ \Delta P_{S} = \frac{1}{2}\rho_{0}{V_{c}^{2} - \frac{1}{2}\rho_{0}{V_{ic}}^{2} \text{ (low Mach only)}} \]

Solve for \(V_c\)

\[ \Delta V_{pc} = V_c - V_{ic} \]
...
Correct \(V_i\) for instrument error corrections (\(\Delta V_{ic} \)) using

\[V_{ic} = V_i + \Delta V_{ic} \]

\[\Delta V_{pc} = V_c - V_{ic} \]

To determine altimeter error assume \(\Delta P_T = 0\)

\[\frac{1}{2} \rho_0 \left[ V_c^2 - V_{ic}^2 \right] = +\Delta P_s \]

\[\Delta P_s = +\rho g \Delta H \]

If \(\Delta P_T \ne 0\), then \(\Delta H_{pc} = + \frac{\Delta P_s - \Delta P_T}{\rho g} \)
...
\(\Delta P_s = \Delta P_{s_{A/C}} - \Delta P_{s_{REF}} \)

\(\Delta H_{pc} = +\frac{\Delta P_s}{\rho g} = \textrm{altimeter correction} \)  <!--- Probably not needed to define this here again --->

\(\Delta P_T - \Delta P_s = \Delta P_D = q_{ic} - q_c \textrm{ assuming } \Delta P_T = 0 \textrm{ , } M < 0.2 \)  <!--- Unclear --->

If pitot errors do exist, then they must be included in calculations for \(\Delta V_{pc}\) (see flowchart). <!--- which one??? --->
...
If exact (\(\pm 10^{\circ}\)) winds are calculated inflight, you can fly one pass directly into/away from the wind
...
<!--- FIXME: This should be defined elsewhere --->
  * Requirements
    * Maximum error at sea level must be less than \(\pm 30 \textrm{ft}/100 \textrm{kts}\) between \(1.3 V_{SO}\) and \(1.8 V_{S1}\) 
...
Landing configuration: \(\Delta H_{pc} \pm 30\) ft.
