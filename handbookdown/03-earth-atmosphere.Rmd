...
|                                                                        |             | \(13.22 \times 10^{24}\)                                                                     | lb        |
| rotation rate                                                          | \(\omega\)  | \(7.292115 \times 10^{−5}                                                                    | rad/sec   |
...
For any centrifugal relief calculations associated with aircraft performance, it is sufficiently exact (g ±0.00004 *m*/*s*^2^) to use the average earth radius. An aircraft flying eastward contributes to centrifugal relief while a westbound aircraft diminishes it.
...
\[
g_{lsl} = 9.780327 \left( 1 + 0.005300224 \sin^2{\phi} - 0.000058 \sin^2{2 \phi} \right) \left[ \frac{m}{s^2} \right]
\]
...
| Density                                      | \(\rho\)              | 0.0023769             | slug/ft^3^          |
|                                              |                       | 1.225                | kg/m^3^ (at 15°C) |
| Specific weight                              | \(g_\rho\)            | 0.07647               | sec^2^/ft^4^        |
...
|                             |            | 59        | °F              |
|                             |            | 15        | °C              |
| Density                     | \(\rho_o\) | 0.0023769 | slgs/ft^3^        |
| (at 15°C)                 |            | 0.07647   | lbm/in^3^         |
|                             |            | 1.255     | kg/m^3^           |
...
\(L\) = standard temperature lapse rate = -0.0065 K/m = -0.0019812 K/ft
...
Baro bias = \(H_b\) - *Hp* = *H_b -- (29.92^0.1903^ -- PA^0.1903^)/0.00001313 \[ft\]

where
PA = ambient pressure = {altimeter setting \[in Hg\]^0.1903^ -- *H_b(0.00001313)}^1/0.1903^
