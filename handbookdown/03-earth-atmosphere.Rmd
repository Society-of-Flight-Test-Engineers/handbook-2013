...
|----------------------------------------|---------|--------------------------------------|--------|
| Avogadros number                       | \(N_o\) | \(6.022\,169 \times 10^{23}\) | molecules/mole |
...
|------------------------------------------------------------------------|-------------|----------------------------------------------------------------------------------------------|-----------|
| Std Earth gravitational acceleration                                   | \(g_o\)     | 9.8066                                                                                       | m/s^2^    |
|                                                                        |             | 32.174                                                                                       | ft/s^2^   |
...
\[
g_{lsl} = 9.780 327 \left( 1 + 0.005 300 224 \sin^2{\phi} - 0.000\,058 \sin^2{2 \phi} \right) \left[ \frac{m}{s^2} \right]
\]
...
\[
g_{alt} = g_{sls}\left( \frac{R}{R + h_G} \right)^2
\]
...
\[
g_{A/C} = \left[ g_{lsl} + \omega^2 R \cos{\phi} \right] \left[ \frac{R}{R + h_G} \right]^2 - \left( \omega + \frac{V_G \sin{\sigma}}{R + h_G} \right)^2 \left(R + h_G\right) \cos{\phi}
\]
...
\[C_{L_{std}} = C_{L_i} \frac{N_{z_{\omega_{std}}}}{N_{Z_{\omega_i}}} \left[\frac{g_{std}}{g_{A/C}} \right] \]

\[C_{D_{std}} = \frac{C^2_{L_{std}}}{\pi A \text{Re}} \]

\[\Delta D = D_{std} - D_t = qS\left[C_{D_{i_{std}}} - C_{D_{i_t}} \right] \]

\[\dot{W}_{f_{std}} = \dot{W}_{f_i} + \Delta D \cdot TFSC \]
...
\(\dot{W}_{f_{std}}\) = standard day fuel flow
...
|                                              |                       | 1716                 | lb(ft)/slgs(R)    |
|                                              |                       | 3089.7               | lb(ft)/slgs(K)    |
| Speed of sound                               | \(a_o \sqrt{\theta}\) | 49.02 \(\sqrt{T_R}\) | ft/sec              |
...
| Specific heat capacity at 59 °F (=\(T_o\)) |                       |                      |                     |
...
| Geopotential     | \(h\)   | Required because \(g\) changes with height. If potential energy is calculated using sea level weight ( \(W_{SL} = mg_o\)) instead of actual weight (\(W = mg\)), then the altitude must be lower. \(h = W/W_{SL} h_G \) |
...
1. The air is a perfect gas and obeys the equation of state, \(P = \rho gRT\), where \(R\) = 53.35 ft lb/°R lbm
1. The gravitational field decreases with altitude
1. Hydrostatic equilibrium exists (\(\Delta p = −\rho g_o \Delta h\))

### Standard Day Sea Level Atmospheric Conditions {-}

| Parameter                   | Symbol     | Value     | Unit              |
|-----------------------------|------------|-----------|-------------------|
| Pressure                    | \(P_o\)    | 2116.22   | lb/ft^2^          |
|                             |            | 14.696    | lb/in^2^          |
|                             |            | 29.921    | in Hg             |
|                             |            | 1013.25   | HPa (mb)          |
|                             |            | 101 325    | Pa                |
| Temperature                 | \(T_o\)    | 288.15    | K                 |
|                             |            | 518.67    | R                 |
|                             |            | 59        | °F              |
|                             |            | 15        | °C              |
| Density                     | \(\rho_o\) | 0.002 3769 | slgs/ft^3^        |
| (at 15°C)                 |            | 0.076 47   | lbm/in^3^         |
|                             |            | 1.255     | kg/m^3^           |
| Speed of Sound              | \(a_o\)    | 1116.45   | ft/sec            |
|                             |            | 661.478   | KTAS              |
|                             |            | 761.14    | mph               |
|                             |            | 340.294   | m/sec             |
| Acceleration Due to Gravity | \(g_o\)    | 32.174    | ft/sec^2^         |
|                             |            | 9.806 65   | m/sec^2^          |
...
Below 36 089 ft (11 000 m) and pressure less than 22636 Pa
...
Between 36 089 ft and 65 616 ft (11 0000 m and 20 000 m) the standard day temperature is a constant 216.65 K, therefore:
...
Below the tropopause (ambient pressure greater than 472.683 psf or 22632 Pa)

\[
H_p \left[ \text{ft} \right] = \frac{ 1-\left( \frac{P_a}{P_o} \right)^{0.190\,2632} }{6.875\,5856 \times 10^{-6}}
\]

In the troposphere (ambient pressure between 114.347 and 472.683 psf or between 5475 and 22 632.1 Pascal)

\[
H_p \left[ \text{ft} \right] = 36\,089+ \frac{ \ln \left( \frac{P_a}{P_o} \right) + 1.498\,966 }{ 0.000\,048\,063 }
\]
...
\[
\begin{align}
\phi_{\text{scale}} \left\{ \phi = 0 \right\} &= 110\,574 \text{m} = 362\,776.6 \text{ft} = 59.705\,18 \text{nm} \\
\lambda_{\text{scale}} \left\{ \phi = 0 \right\} &= 111\,319.5 \text{m} = 365\,221.4 \text{ft} = 60.107\,72 \text{nm} \\
\end{align}
\]
...
\[
\begin{align}
\phi_{\text{scale}} &= \frac{\phi_{\text{scale}} \left\{ \phi = 0 \right\} }{ \left\lbrack \left( 1 - e^{2} \right)  \sin^{2}(\phi) \right\rbrack^{1.5} } \\
\\
\lambda_{\text{scale}} &= \frac{\lbrack \lambda_{\text{scale}} \left\{ \phi = 0 \right\} \rbrack \cdot \cos{\phi}}{\sqrt{ \left(1 - e^2 \right) \sin^{2}{\phi}}} \\
\end{align}
\]
...
\[
\begin{align}
Y = \phi_{\text{scale}} \left(\phi_1 - \phi_0 \right) \\
X = \lambda_{\text{scale}} \left(\lambda_1 - \lambda_0 \right) \\
\end{align}
\]
...
\[
\begin{align}
X &= \phi_{\text{scale}} \left(\phi - \phi_0 \right)\sin{\psi} + \lambda_{\text{scale}} \left(\lambda - \lambda_0 \right)\cos{\psi} \\
Y &= \phi_{\text{scale}} \left(\phi - \phi_0 \right)\cos{\psi} - \lambda_{\text{scale}} \left(\lambda - \lambda_0 \right)\sin{\psi} \\
\end{align}
\]
...
\[h_G = H_b + \frac{T_{\text{apt}} - T_0}{L - Hp_{\text{apt}} } \ln \left[ 1 + \frac{L \left( H_p - H_{p_{\text{apt}}} \right) }{T_0 + L H_{p_{\text{apt}}}} \right]\]
